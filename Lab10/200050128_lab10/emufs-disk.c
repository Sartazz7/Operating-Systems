...
void read_inode(int mount_point, int inodenum, struct inode_t *inodeptr){
	...
	*/
	...
	char tempBuf[BLOCKSIZE];
	readblock(mounts[mount_point].device_fd,inodenum/16+1,tempBuf);

	memcpy(inodeptr,&tempBuf[16*(inodenum%16)],sizeof(struct inode_t));
}
...
void write_inode(int mount_point, int inodenum, struct inode_t *inodeptr){
	...
	*/
	...
	memcpy(&tempBuf[16*(inodenum%16)],inodeptr,sizeof(struct inode_t));
	
	writeblock(mounts[mount_point].device_fd,inodenum/16+1,tempBuf);
}
