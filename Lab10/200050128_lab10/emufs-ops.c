...
int emufs_delete(int dir_handle, char* path){
    ...
    */
    ...
    int inodenum = return_inode(mount_point,dir[dir_handle].inode_number,path);
    if(inodenum < 0 || inodenum >= MAX_INODES){
        return -1;
    }

    int parent = delete_entity(mount_point,inodenum);
    struct inode_t inode;
...
int emufs_create(int dir_handle, char* name, int type){
    ...
    */
    int mount_point = dir[dir_handle].mount_point;
    struct inode_t inode, childInode;

    read_inode(mount_point,dir[dir_handle].inode_number,&inode);
    for(int i=0; i<4; i++){
        if(inode.mappings[i] == -1)continue;
        read_inode(mount_point,inode.mappings[i],&childInode);
        if(childInode.type == type && strcmp(childInode.name,name) == 0){
            return -1;
        }
    }
    for(int i=0; i<4; i++){
        ...
        read_inode(mount_point,inodenum,&childInode);
        strcpy(childInode.name,name);
        childInode.type = type;
        childInode.parent = dir[dir_handle].inode_number;
        childInode.size = 0;
        for(int j=0; j<4; j++){
            childInode.mappings[j] = -1;
        }
        inode.mappings[i] = inodenum;
        inode.size += 1;
        write_inode(mount_point,inodenum,&childInode);
...
int open_file(int dir_handle, char* path){
    /*
        ...
        * Return value: -1, error
                         1, success  // should return file_handle
    */
    int mount_point = dir[dir_handle].mount_point;

    int inodenum = return_inode(mount_point,dir[dir_handle].inode_number,path);
    int file_handle = alloc_file_handle();
    if(inodenum < 0 || file_handle < 0){
        return -1;
    }
...
int emufs_read(int file_handle, char* buf, int size){
    ...
    */
    ...
    int bufOffset = 0, tempOffset = files[file_handle].offset;

    struct inode_t inode;
    read_inode(mount_point,files[file_handle].inode_number,&inode);

    if(size > MAX_FILE_SIZE*BLOCKSIZE){
        return -1;
    }

    for(int i=0; i<4; i++){
        if(inode.mappings[i] == -1)continue;
        if(tempOffset >= BLOCKSIZE){
            tempOffset -= BLOCKSIZE;
            continue;
        }
        if(bufOffset >= size)continue;
        read_datablock(mount_point,inode.mappings[i],tempBuf);
        int sz = size-bufOffset;
        if(sz > BLOCKSIZE-tempOffset)sz = BLOCKSIZE-tempOffset;
        memcpy(&buf[bufOffset],&tempBuf[tempOffset],sz);
        bufOffset += sz;
        tempOffset = 0;
    }

    files[file_handle].offset += bufOffset;
    return 1;
}
...
int emufs_write(int file_handle, char* buf, int size){
    ...
    */
    ...
    for(int i=0; i<4; i++){
        ...
        if(bufOffset >= size)continue;
        int blocknum = inode.mappings[i];
        if(blocknum == -1)blocknum = alloc_datablock(mount_point);
        read_datablock(mount_point,blocknum,tempBuf);
        int sz = size-bufOffset;
        if(sz > BLOCKSIZE-tempOffset)sz = BLOCKSIZE-tempOffset;
        // printf("bufOffset: %d sz: %d tempOffset: %d\n",bufOffset,sz, tempOffset);
        // printf("%d %s\n",i,tempBuf);
        memcpy(&tempBuf[tempOffset],&buf[bufOffset],sz);
        // printf("%d %s\n",i,tempBuf);
        write_datablock(mount_point,blocknum,tempBuf);
        inode.mappings[i] = blocknum;
        bufOffset += sz;
        tempOffset = 0;
    }
    ...
    files[file_handle].offset += bufOffset;
    inode.size += bufOffset;
    write_inode(mount_point,files[file_handle].inode_number,&inode);
    return 1;
}
