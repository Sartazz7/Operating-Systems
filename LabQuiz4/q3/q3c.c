// num theads are not more than 8
#include <limits.h>
#include <stdbool.h>
#include <stdio.h>
#include <stdlib.h>
...
long numTrans;
pthread_mutex_t lock[NUM_CUSTOMERS],mutex;
...
void* executeTransaction(void*arg){
    ...
    for(int i=id;i<numTrans;i+=numThreads){
        sleep(sleeptime); // do not remove this sleep statement or you will lose marks
        
        if(allTransactions[i].from < allTransactions[i].to){
            pthread_mutex_lock(&lock[allTransactions[i].from]);
            pthread_mutex_lock(&lock[allTransactions[i].to]);
        }
        else{
            pthread_mutex_lock(&lock[allTransactions[i].to]);
            pthread_mutex_lock(&lock[allTransactions[i].from]);
        ...
        balance[allTransactions[i].from]-=allTransactions[i].amount;
        balance[allTransactions[i].to]+=allTransactions[i].amount;

        if(allTransactions[i].from < allTransactions[i].to){
            pthread_mutex_unlock(&lock[allTransactions[i].from]);
            pthread_mutex_unlock(&lock[allTransactions[i].to]);
        }
        else{
            pthread_mutex_unlock(&lock[allTransactions[i].to]);
            pthread_mutex_unlock(&lock[allTransactions[i].from]);
